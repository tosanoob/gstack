...
- **Skills now discoverable via natural language.** All 12 skills that were missing explicit trigger phrases now have them — say "deploy this" and Gemini finds `/ship`, say "check my diff" and it finds `/review`. Following Anthropic's best practice: "the description field is not a summary — it's when to trigger."

## [0.6.4.0] - 2026-03-17

### Added

- **`/plan-design-review` is now interactive — rates 0-10, fixes the plan.** Instead of producing a report with letter grades, the designer now works like CEO and Eng review: rates each design dimension 0-10, explains what a 10 looks like, then edits the plan to get there. One ask_user tool per design choice. The output is a better plan, not a document about the plan.
...
- **Completeness scoring**: every ask_user tool option now shows a completeness
  score (1-10), biasing toward the complete solution
- **Dual time estimates**: effort estimates show both human-team and Gemini+gstack time
  (e.g., "human: ~2 weeks / Gemini: ~1 hour") with a task-type compression reference table
...
- **Review completeness gaps**: `/review` now flags shortcut implementations where the
  complete version costs <30 min Gemini CLI time
...
- **CEO + Eng review dual-time**: temporal interrogation, effort estimates, and delight
  opportunities all show both human and Gemini CLI time scales
...
- Rewrote EXPANSION mode's Step 0D to include opt-in ceremony — distill vision into discrete proposals, present each as ask_user tool.
...
- Merged ELI16 rules into base ask_user tool format — one format instead of two, no `_SESSIONS >= 3` conditional.
...
- Stale servers from global install no longer shadow local changes — removed legacy `~/.gemini/skills/gstack` fallback from `resolveServerScript()`
...
- Browser interaction guidance in CLAUDE.md — prevents Gemini from using mcp\_\_gemini-in-chrome\_\_\* tools
...
- Legacy fallback to `~/.gemini/skills/gstack/browse/src/server.ts`
