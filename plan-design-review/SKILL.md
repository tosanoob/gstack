...
### 0D. Focus Areas
ask_user tool: "I've rated this plan {N}/10 on design completeness. The biggest gaps are {X, Y, Z}. Want me to review all 7 dimensions, or focus on specific areas?"
...
5. ask_user tool if there's a genuine design choice to resolve
...
**STOP.** ask_user tool once per issue. Do NOT batch. Recommend + WHY. If no issues, say so and move on. Do NOT proceed until user responds.
...
**STOP.** ask_user tool once per issue. Do NOT batch. Recommend + WHY.
...
```
  ...
  ...
```
Each decision = one ask_user tool with recommendation + WHY + alternatives. Edit the plan with each decision as it's made.

## CRITICAL RULE — How to ask questions
Follow the ask_user tool format from the Preamble above. Additional rules for plan design reviews:
...
* **Escape hatch:** If a section has no issues, say so and move on. If a gap has an obvious fix, state what you'll add and move on — don't waste a question on it. Only use ask_user tool when there is a genuine design choice with meaningful tradeoffs.
...
### TODOS.md updates
After all review passes are complete, present each potential TODO as its own individual ask_user tool. Never batch TODOs — one per question. Never silently skip this step.
...
If any ask_user tool goes unanswered, note it here. Never silently default to an option.
