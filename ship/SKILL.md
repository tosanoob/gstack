...
2. **If no override exists,** use ask_user tool:
   ...
   - For Design Review: run `eval $(~/.gemini/skills/gstack/bin/gstack-diff-scope <base> 2>/dev/null)`. If `SCOPE_FRONTEND=true` and no design review (plan-design-review or design-review-lite) exists in the dashboard, mention: "Design Review not run — this PR changes frontend code. The lite design check will run automatically in Step 3.5, but consider running /design-review for a full visual audit post-implementation." Still never block.
...
**If NO runtime detected** (no config files found): Use ask_user tool:
...
1. Read `.gemini/skills/review/checklist.md`. If the file cannot be read, **STOP** and report the error.
...
```bash
eval $(~/.gemini/skills/gstack/bin/gstack-diff-scope <base> 2>/dev/null)
```
...
2. **Read `.gemini/skills/review/design-checklist.md`.** If the file cannot be read, skip design review with a note: "Design checklist not found — skipping design review."
...
6. **If ASK items remain,** present them in ONE ask_user tool:
   - List each with number, severity, problem, recommended fix
   - Per-item options: A) Fix  B) Skip
   - Overall RECOMMENDATION
   - If 3 or fewer ASK items, you may use individual ask_user tool calls instead
...
Read `.gemini/skills/review/greptile-triage.md` and follow the fetch, filter, classify, and **escalation detection** steps.
...
**VALID & ACTIONABLE:** Use ask_user tool with:
...
**VALID BUT ALREADY FIXED:** Reply using the **Already Fixed reply template** from greptile-triage.md — no ask_user tool needed:
- Include what was done and the fixing commit SHA
- Save to both per-project and global greptile-history (type: already-fixed)

**FALSE POSITIVE:** Use ask_user tool:
...
Read `.gemini/skills/review/TODOS-format.md` for the canonical format reference.

**1. Check if TODOS.md exists** in the repository root.

**If TODOS.md does not exist:** Use ask_user tool:
...
**If disorganized** (missing priority fields, no component groupings, no Completed section): Use ask_user tool:
...
🤖 Generated with [Gemini CLI](https://gemini.com/gemini-code)
EOF
)"
```
...
- **Never ask for confirmation** except for MINOR/MAJOR version bumps and pre-landing review ASK items (batched into at most one ask_user tool).
